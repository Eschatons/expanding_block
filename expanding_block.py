...
output: 
    ...
    try:
        ...
        if shape[2] == 1:   # image is grayscale already
            ...
            baseImg = color.gray2rgb(baseImg)
        else:
            raise valError
    ...
    for n in range(init.numBuckets):
        try:
            ...
            if n == 0:
                buckets[n] = groups[n]+groups[n+1]
            elif n == init.numBuckets-1:
                buckets[n] = groups[n-1]+groups[n]
                
    print(str(buckets[0][0]))
    
    '''
    note that because of python's behavior,
    bucket[0] = [group[N], group[0], group[1]]
    bucket [N] = [group[N-1], group[N], group[0]].
    This will cause minor additional computational overhead,
    but it's 'cleaner' than a lot of if-else statements
    '''
    
    """
    # TEST:
    if DEBUG:
        bucketLength = [len(bucket) for bucket in buckets]
        print('The number of elements in each bucket is:')
        print(bucketLength)
    """
    ...
    imgOut = write_mask(mask, baseImg)
